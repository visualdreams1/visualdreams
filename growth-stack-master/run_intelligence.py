...
def main():
    ...
    sales=autopilot()
    monitor=check()
    print(json.dumps({"research":"complete","intelligence":intel,"business_discovery":{"businesses":len(discovered)},"contact_verification":{"leads":len(verified),"with_contacts":sum(bool(x.get("channels")) for x in verified)},"multichannel":multi,"lead_acquisition":leads,"sales":sales,"monitor":monitor},indent=2))
